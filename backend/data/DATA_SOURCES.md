...
This project merges **multiple Roboflow Universe** exports in **YOLOv8** format. Download the zip(s), extract them, and place the folders under `backend/data/raw/` using the **exact folder names** referenced in `configs/classes.yaml`:
...
| `ds_veg_seg` | `Vegetables.v5-danya.yolov8` | [vegetables-f4ale / v5](https://universe.roboflow.com/labs-hsbu3/vegetables-f4ale/dataset/5) (**instance segmentation** polygons; Broccoli/Cucumber/Tomato) |
...
本项目合并多个 **Roboflow Universe** 的 **YOLOv8** 格式数据集。下载 zip 并解压到 `backend/data/raw/`，**目录名**须与 `configs/classes.yaml` 中 `path` 一致：
...
| `ds_veg_seg` | `Vegetables.v5-danya.yolov8` | [vegetables-f4ale / v5](https://universe.roboflow.com/labs-hsbu3/vegetables-f4ale/dataset/5)（**实例分割**多边形；西兰花/黄瓜/番茄） |
